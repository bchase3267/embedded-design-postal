...

{
    ...
    /* Drive motors */
    pwm_write(LEFT_DRIVE_PORT, left_power);//pwm_write(LEFT_DRIVE_PORT, -left_power);
    pwm_write(RIGHT_DRIVE_PORT, -right_power);//pwm_write(RIGHT_DRIVE_PORT, right_power);
...

{
   ...
   // bumps[2] = 0;
    ...
    // pauses if signal lost
    for(i=0; (rc_read_data(6) != -127) && (i < 400);i++)
    {
        delay_msec(50);
        //bumps[0] = io_read_digital(LEFT_DRIVE_PORT)
        //printf(bumps);
